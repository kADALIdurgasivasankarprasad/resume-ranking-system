...
if st.button("Compare & Rank Resumes"):
    if uploaded_files and job_description:
        ...
        # Compute Dot Product Similarity (Alternative to Cosine Similarity)
        similarity_scores = torch.matmul(resume_embeddings, job_desc_embedding)  # Compute dot product

        # Normalize the similarity scores (convert to percentage)
        similarity_scores = similarity_scores / similarity_scores.max() * 100
        similarity_scores = similarity_scores.cpu().numpy().round(2)  # Convert to percentage
